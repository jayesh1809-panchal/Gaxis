...
class ApplicationLifecycleManager {
  ...
  async registerApplication(config) {
    ...
    const {
      record,
      clientSecret
    } = await this.provisioner.provision(config);

    // 2. Save to central registry
    await this.registry.save(record);
    this.sdk.events.emitEvent('application:registered', {
  ...
  async updateApplication(applicationId, updates) {
    ...
    this.sdk.events.emitEvent('application:updated', {
      applicationId
    });
    return updatedApp;
  }
  async deactivateApplication(applicationId) {
    const app = await this.updateApplication(applicationId, {
      status: 'INACTIVE'
    });
    this.sdk.events.emitEvent('application:deactivated', {
      applicationId
    });
    return app;
  }

  ...
  router.get('/callback', async (req, res) => {
    ...
    try {
      ...
      if (options.successRedirect) {
        res.cookie("gaxis_session", session.sessionId, {
          httpOnly: false
        });
        res.cookie("gaxis_user", JSON.stringify(localUser), {
          httpOnly: false
        });
  ...
  router.post('/logout', async (req, res) => {
    try {
      ...
      if (logoutUrl) {
        ...
        res.json({
          success: true,
          message: "Local logout processed."
        });
      }
    } catch (error) {
...
expressProvider.requireRole = (sdkInstance, localRole) => {
  return async (req, res, next) => {
    ...
    if (!localUserId) return res.status(401).json({
      error: "Unauthorized"
    });

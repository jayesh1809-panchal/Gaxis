...
class TokenManager {
  ...
  async exchangeCode(code, codeVerifier) {
    try {
      ...
      this.sdk.events.emitEvent(this.sdk.events.events.AUTH_SUCCESS, {
        tokenData
      });
      return tokenData;
    } catch (error) {
      this.sdk.logger.error(`Token exchange failed: ${error.message}`);
      this.sdk.events.emitEvent(this.sdk.events.events.AUTH_FAILURE, {
        error: error.message
      });
  ...
  async validateIdToken(idToken) {
    try {
      ...
      throw new AuthenticationError('Invalid ID Token signature or claims', {
        originalError: error
      });
    }
  }

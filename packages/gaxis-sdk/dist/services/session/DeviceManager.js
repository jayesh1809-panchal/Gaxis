...
class DeviceManager {
  ...
  async registerDevice(req) {
    ...
    if (!existingDeviceStr) {
      ...
      this.sdk.events.emitEvent(SessionEvents.DEVICE_REGISTERED, {
        deviceId
      });
  ...
  async trustDevice(deviceId) {
    ...
    if (dev) {
      ...
      this.sdk.events.emitEvent(SessionEvents.DEVICE_TRUSTED, {
        deviceId
      });
    }
  }
  async blockDevice(deviceId) {
    ...
    if (dev) {
      dev.status = 'BLOCKED';
      await this.sdk.storage.set(this._getDeviceKey(deviceId), JSON.stringify(dev));
      this.sdk.events.emitEvent(SessionEvents.DEVICE_BLOCKED, {
        deviceId
      });
    }
  }

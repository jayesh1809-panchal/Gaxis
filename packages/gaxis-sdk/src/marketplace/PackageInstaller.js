...
class PackageInstaller {
    ...
    async install(packageName) {
        ...
        try {
            ...
            const { application, clientSecret } = await this.sdk.application.registerApplication(integrationManifest);
            ...
            this.sdk.events.emitEvent('package:installed', { packageName });

            return application;
        } catch (error) {
            this.sdk.logger.error(`Install Failed for ${packageName}: ${error.message}`);
            this.sdk.events.emitEvent('package:failed', { packageName, error: error.message });
            throw error;
        }
    }
}
